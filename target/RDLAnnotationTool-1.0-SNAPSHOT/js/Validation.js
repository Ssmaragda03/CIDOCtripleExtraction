...
function clearValidationHighlights() {
  ...
  tbody.querySelectorAll("tr").forEach(tr => {
    ...
    tr.querySelectorAll("td").forEach(td => {
      td.classList.remove("err-domain", "err-range", "err-prop");
...
function applyValidationToRdfTable(annotations) {
  ...
  (annotations || []).forEach(function(a){
    ...
    rows.forEach(function(tr){
      ...
      if (onlyProp) {
        if (errors.indexOf("property_error") !== -1) {
...
function validateRdfTableWarnings() {
  ...
  const prefixes = (typeof getPrefixesFromUI === "function") ? getPrefixesFromUI() : {};
  ...
  rows.forEach((tr, index) => {
    ...
    const predicate = tds[1].innerText.trim();
    const object = tds[2].innerText.trim();
...
function hasUnknownPrefix(value, prefixes) {
  ...
  // full URI ή literal -> όχι prefix check
