...
//          console.log("Entities received:", entities);
          ...
          document.getElementById("promptPreview1").textContent = await loadRelationPromptTemplate();
            ...
            // ΔΙΑΧΕΙΡΙΣΗ ΣΦΑΛΜΑΤΩΝ (API Key, Quota, κλπ)
...
function goback(){ // goes back in entity recognition
  show("EntityRecognition");
  setMode('relation','text');
  hide("RelationExtraction");
  document.getElementById("response").style.display = "none";
  document.getElementById('EntityRecognition').scrollIntoView({behavior: 'instant', block: 'start'});
  document.getElementById("back-btn-entity").style.display = "none";
}


function gobackRelation(){ // goes back in entity recognition - clears the previous status 
  document.getElementById("tabEntities").style.display = "block";
  show("RelationExtraction");
  document.getElementById("back-btn-entity").style.display = "block";
  document.getElementById("back-btn-relation").style.display = "none";
